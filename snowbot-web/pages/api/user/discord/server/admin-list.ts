...
export default async function handler(
    ...
    const response = 
        await fetch('https://discord.com/api/v8/users/@me/guilds', { headers: { Authorization: `Bearer ${accessToken}` }});
    if (!response.ok) {
        return res.status(500).send('Failed to fetch guilds');
    }

    const guilds: any = await response.json();
    const adminGuilds = guilds.filter((guild: any) => (guild.permissions & 0x8) !== 0);
    ...
    res.status(200).json(formattedGuilds);
}
