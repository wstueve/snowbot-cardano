...
import { CreateNextAirdropBatchCommand } from "../../../../components/type/airdrop";
import {
  createNextAirdropBatch,
...
export default async function handler(
  ...
  if (req.method === 'POST') {
    const command = {
      ...req.body as CreateNextAirdropBatchCommand
    };
    ...
    const tx = await createNextAirdropBatch(userId, command);
    if (!tx) {
      res.status(400).json({ message: "Bad Request." });
      return;
    }
