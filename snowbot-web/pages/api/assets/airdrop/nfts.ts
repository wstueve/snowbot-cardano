...
import { AddBulkAirdropNftCommand, CreateAirdropCommand } from "../../../../components/type/airdrop";
import { getRemainingAirdropNftsCount } from "../../../../components/logic/airdrop";
...
export default async function handler(
  ...
  if (req.method === 'GET') {
    const airdropId = req.query.id as string;
    
    const remainingNftsCount = await getRemainingAirdropNftsCount(userId, airdropId);
    res.status(200).json({ remainingNftsCount });
    return
  }
  ...
  if (req.method === 'POST') {
    ...
    // await addNftsToAirdrop(command);
    res.status(503).json({ "message": "Service unavailable - Needs upgraded" });
    return
  }

  res.status(405).send({ message: 'Invalid request method. Aridrop index' })
}
